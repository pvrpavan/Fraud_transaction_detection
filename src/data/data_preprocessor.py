...
class DataPreprocessor:
    ...
    def _clean_data(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        # Pre-transaction balances (oldbalanceOrg, oldbalanceDest) are also
        # dropped because in the PaySim dataset they create near-perfect
        # separation of fraud vs legitimate transactions, leading to
        # artificially inflated accuracy that doesn't generalize.
        balance_cols = [
            "newbalanceOrig", "newbalanceDest",
            "oldbalanceOrg", "oldbalanceDest",
        ]
        existing_balance_cols = [c for c in balance_cols if c in df.columns]
        if existing_balance_cols:
            logger.info(f"Dropping balance columns to prevent data leakage: {existing_balance_cols}")
            df = df.drop(columns=existing_balance_cols)
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Uses a minimal, interpretable feature set to avoid overfitting
        and keep the model simple and generalizable.
        """
